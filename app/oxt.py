...
def oxts2pose(oxts, drive):
    
    if(len(oxts) == 0):
        return ""
    print(len(oxts), drive)
...
def load_oxts_lite_data(base_dir, frames):
    ...
    for frame in (frames):
        ...
        if( os.path.isfile(filename) ):
            #print(filename, np.loadtxt(filename))
            oxts.append(np.loadtxt(filename))
            
        else:
            folder_name = fname.split("_")
            if(len(folder_name)> 1):
                main_folder = folder_name[0]
                file_name =  folder_name[1][:4]
                linenumber = int(folder_name[1][4:])
                '''
                if(linenumber>0):
                    
                    filename = os.path.join(base_dir,'oxts/'+main_folder+'/', file_name + '.txt')

                    _data = np.loadtxt(filename)
                    
                    for _d in tqdm(_data):
                        
                        
                #print("inner", filename, linenumber)
                #print(_data)
                '''
                ...
                _data = np.loadtxt(filename)
                oxts.append(_data[linenumber-1])
                #print(filename, (_data[linenumber-1]))
