...
def data_preprocessing(drivename, fname , max_point_num):
    batch_size = 2048
    block_size = 1000
    grid_size = 0.25
    ...
    print(xyzi.shape, len((xyzi[:,0] >= -5 ).nonzero()[0]) )
...
def main(FLAG):
    ...
    with tf.Session() as sess:
        ...
        for fname in all_files:
            ...
            output_location = os.path.join(ROOT_DIR, "output/"+drivename+"_"+fname+"axcrf.bin")
            
            if os.path.isfile( output_location ) :
                continue

            label_length, data, data_num, indices_split_to_full, item_num, all_label_pred, indices_for_prediction = data_preprocessing(drivename, fname , max_point_num)
