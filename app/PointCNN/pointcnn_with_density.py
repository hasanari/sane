...
def xconv_only_feature(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation, depth_multiplier,
    ...
    nn_pts = tf.gather_nd(fts, indices, name=tag + 'nn_pts')  # (N, P, K, 1)
    nn_pts_center = tf.expand_dims(fts, axis=2, name=tag + 'nn_pts_center')  # (N, P, 1, 1)
    nn_pts_local = tf.subtract(nn_pts, nn_pts_center, name=tag + 'nn_pts_local')  # (N, P, K, 1)
...
class PointCNN:
    def __init__(self, points, __features, is_training, setting):
        ...
        if __features is None:
            self.layer_fts = [__features]
        else:
            features = tf.reshape(__features[:,:,0:1], (N, -1, 4 - 3), name='features_reshape') #reflectance only
            C_fts = xconv_params[0]['C'] // 2
            features_hd = pf.dense(features, C_fts, 'features_hd', is_training)
            self.layer_fts = [features_hd]

        
        fts_xconv_feature = None
        
        
        density_features = tf.reshape(__features[:,:,1:2], (N, -1, 1), name='density_reshape') #density only
        C_fts = xconv_params[0]['C'] // 2
        density_features_hd = pf.dense(density_features, C_fts, 'density_features', is_training)
        ...
        for layer_idx, layer_param in enumerate(xconv_params):
            ...
            if(layer_idx == 0):
                print("fts_xconv_feature")

                fts_xconv_feature = xconv_only_feature(pts, density_features_hd, qrs, "feature_only_"+tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation,
                                  depth_multiplier, sorting_method, with_global)
        ...
        self.fc_layers = [fts_xconv_feature, self.layer_fts[-1]]
